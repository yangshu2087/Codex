---
name: gpt-image
description: Explicit-use GPT Image 2 prompt-gallery and reference-image editing skill for OpenAI image workflows. In this Codex workspace, prefer it when the user explicitly asks for `gpt-image`, GPT Image 2 prompt-atlas patterns, reference-image edit guidance, or the upstream `wuyoscar/gpt_image_2_skill` examples. Live generation/editing requires `OPENAI_API_KEY`; without a key, use the references for prompt drafting only. Writes PNG/JPEG/WebP to disk when the CLI is invoked.
---
...
- This copy is vendored under `/Users/yangshu/Codex/vendor/skills/gpt-image` for reviewed, explicit use.
- In this workspace, do **not** treat it as the default image tool. Prefer the built-in `imagegen` skill for normal OpenAI image API work when live API use is actually approved.
- If `OPENAI_API_KEY` is missing, use `references/gallery.md`, the category files under `references/`, and `references/craft.md` for prompt drafting and examples only; do not claim that live image generation is available.
- The local `scripts/generate.py` shim is patched to run from the vendored layout by loading `src/gpt_image_cli` from inside this skill directory.
