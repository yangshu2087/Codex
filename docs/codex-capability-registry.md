...
|---|---|---|---|---|
| Default model + reasoning | `~/.codex/config.toml` | Active, needs review | user | Current default is `gpt-5.4` with top-level `high` reasoning; keep `xhigh` only in the `deep` profile. |
...
| `architecture-decision-review` | `.agents/skills/architecture-decision-review` | Active, explicit-only | workspace | Architecture option comparison, rollout, rollback, and risk evidence. |
| `backend-api-contract-review` | `.agents/skills/backend-api-contract-review` | Active, explicit-only | workspace | Backend/API contract, permissions, consistency, and targeted regression checks. |
| `product-ux-flow-review` | `.agents/skills/product-ux-flow-review` | Active, explicit-only | workspace | Product/UX flow, state coverage, copy, accessibility, and visual verification. |
...
- `docs/codex-quality-lanes.md` is now the source of truth for requirements, architecture, front-end, backend/API, code review, and research quality gates.
- `docs/codex-github-skill-watchlist-2026-04-14.md` tracks third-party candidates without default activation.

## Immediate governance watchlist

1. Keep top-level default reasoning at `high`; use `deep` profile for `xhigh` architecture or long reasoning work.
2. Keep Google Drive plugin naming on `google-drive@openai-curated`; avoid reintroducing the stale `googledrive` alias.
