...
for f in "$AUDIT_SCRIPT" "$HANDOFF_SCRIPT" "$REPO_ROOT/scripts/skill-smoke.sh" "$REPO_ROOT/scripts/codex-runtime-health.sh" "$REPO_ROOT/scripts/codex-maint-weekly.sh" "$QUALITY_LANE_SMOKE"; do
  if [[ ! -f "$f" ]]; then
...
echo "[1/5] shell syntax checks passed"
...
print('[2/5] skill frontmatter check passed')
PY
...
echo "[3/5] skill-audit smoke passed"

"$QUALITY_LANE_SMOKE" >/tmp/codex-quality-lane-smoke.out
grep -q "All Codex quality lane smoke checks passed" /tmp/codex-quality-lane-smoke.out
echo "[4/5] quality lane smoke passed"
...
echo "[5/5] agent handoff refresh smoke passed"
echo ""
